 ...
 */
public class _MonthYearPicker extends _PanelComponent implements BindableComponent<Date>, DateSelected {

    public static _MonthYearPicker from() {
        return new _MonthYearPicker();
    }

    private final SimpleDateFormat SDF_MONTH = new SimpleDateFormat("MMMM");
    private final List<MES> months = new ArrayList<>(12);

    public _MonthYearPicker() {
        initComponents();
        createMonths();
        setMonths();
        setActualDate();
    ...
    public void addActionListener(java.awt.event.ActionListener listener) {
        comboBoxMonths.addActionListener((java.awt.event.ActionEvent evt) -> {
            listener.actionPerformed(evt);
        });
        comboBoxYear.addActionListener((java.awt.event.ActionEvent evt) -> {
            listener.actionPerformed(evt);
        });
    }
    ...
    public Year getSelectedYear() {
        return (Year) comboBoxYear.getSelectedItem();
    }
    ...
    @Override
    public Date getDate() {
        return new Date(getSelectedYear().getValue(), getSelectedMonth(), 1);
    }

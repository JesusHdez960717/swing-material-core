...
public class _ClockFace extends JComponent implements MaterialComponent {
    ...
    protected void paintFace(Graphics graphics, int size) {
        ...
        for (i = 0; i < 12; i++) {
            double theta = (6.283185307179586d * ((double) i)) / 12.0d;
            ...
            g2.drawString(str, (float) Math.round((((double) numbers) * Math.cos(theta)) - (g2.getFontMetrics().getStringBounds(str, g2).getWidth() / 2.0d)), (float) Math.round((((double) numbers) * Math.sin(theta)) + ((double) (margin * 2))));
        }
